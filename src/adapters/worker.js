...
onmessage = function(event) {
  ...
  try {
    bidRequest = JSON.parse(event.data);
  } catch (error) {
    console.error(error);
  }
...
function handleResponse(response) {
  ...
  if (!parsed || parsed.error) {
    utils.logError(`Bad response for ut adapter`);
    ...
    //     responses.push({placementCode, bid});
    //   });
    return;
  }
  ...
  parsed.tags.forEach(tag => {
    ...
    const placement = bidRequests[tag.uuid].placementCode;
    responses.push({placement, bid});
  });

  postMessage(JSON.stringify(responses));
